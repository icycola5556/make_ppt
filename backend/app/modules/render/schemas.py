...
class ImageSlotRequest(BaseModel):
    ...
    # === 视觉约束 ===
    visual_style: ImageStyle = Field(
        default=ImageStyle.PHOTO, description="期望的图片风格"
    )
    aspect_ratio: AspectRatio = Field(
        default=AspectRatio.STANDARD, description="期望宽高比"
    )
    color_constraint: ColorConstraint = Field(
        default=ColorConstraint.NO_CONSTRAINT, description="颜色约束"
    )
    min_resolution: tuple[int, int] = Field(
        default=(800, 600), description="最小分辨率(宽,高)"
    )
    ...
    matched_image_url: Optional[str] = Field(
        default=None, description="匹配到的图片URL"
    )
    html_source: Optional[str] = Field(
        default=None, description="生成的HTML中引用的本地图片路径"
    )
...
class LayoutConfig(BaseModel):
    ...
    # === 图片插槽配置 ===
    image_slots: List[Dict[str, Any]] = Field(
        default_factory=list, description="图片插槽位置定义"
    )

    # === 适用场景 ===
    suitable_slide_types: List[str] = Field(
        default_factory=list, description="适用的 slide_type"
    )
    suitable_keywords: List[str] = Field(
        default_factory=list, description="适用的关键词"
    )
...
class RenderResult(BaseModel):
    ...
    # === 图片生成相关 ===
    image_slots: List[ImageSlotRequest] = Field(
        default_factory=list, description="图片插槽列表"
    )
    image_results: List[ImageSlotResult] = Field(
        default_factory=list, description="图片生成结果列表"
    )
    ...
    # 统计信息
    total_pages: int = 0
    layouts_used: Dict[str, int] = Field(
        default_factory=dict, description="使用的布局统计"
    )
