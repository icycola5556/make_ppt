...
- 为后续模块提供充分的结构化信息""")
                    ...
                    try:
                        ...
                        self.logger.emit(
                            ...
                            "3.1",
                            "tool_call_failed",
                            {"error": str(tool_error), "fallback": "disabling_tools"},
                        )
                        # 降级到不使用工具的方案
                        try:
                        ...
                        if isinstance(response, dict):
                            ...
                            try:
                                ...
                                if parsed is None:
                                    ...
                                    raise ValueError(
                                        f"无法从响应中提取有效的JSON。响应预览: {response[:200]}"
                                    )
                        else:
                            raise ValueError(
                                f"意外的响应类型: {type(response)} - 期望dict或str，得到{type(response)}"
                            )
                    ...
                    # 后处理修正常见错误
                    try:
                        parsed = self._post_process_llm_response(
                            parsed, user_text, tool_calls
                        )
                        self.logger.emit(
                            session_id,
                            "3.1",
                    ...
                    parsed, meta = await self.llm.chat_json(
                        INTENT_SYSTEM_PROMPT, user_text, INTENT_SCHEMA_HINT
                    )
                    self.logger.emit(session_id, "3.1", "llm_response", meta)
                    req = TeachingRequest.model_validate(
                        {
                ...
                self._handle_workflow_error(
                    session_id, "3.1", e, {"parsing_method": "llm_extraction"}
                )
        ...
        update_page_distribution(req)
        self.logger.emit(
            session_id, "3.1", "heuristic_parse", req.model_dump(mode="json")
        )
        return req

    async def _design_style(
        self, session_id: str, req: TeachingRequest
    ) -> Tuple[StyleConfig, List[StyleSampleSlide]]:
        ...
        self.logger.emit(
            ...
            "style_base",
            {
                "style_config": cfg.model_dump(mode="json"),
                "style_samples": [s.model_dump(mode="json") for s in samples],
            },
        )
            ...
            try:
                ...
                self.logger.emit(
                    ...
                    "llm_prompt",
                    {
                        ...
                        "user": user_msg,
                        "schema_hint": STYLE_SCHEMA_HINT,
                    },
                )
                parsed, meta = await self.llm.chat_json(
                    STYLE_SYSTEM_PROMPT, user_msg, STYLE_SCHEMA_HINT
                )
                ...
                self._handle_workflow_error(
                    session_id, "3.2", e, {"style_config": cfg.model_dump(mode="json")}
                )
    ...
    async def _generate_outline(
        ...
        if self.llm.is_enabled():
            try:
                ...
                self.logger.emit(
                    session_id, "3.3", "outline_final", outline.model_dump(mode="json")
                )
                return outline
            except Exception as e:
                # LLM调用失败，降级到确定性生成
                self._handle_workflow_error(
                    session_id, "3.3", e, {"fallback_to_deterministic": True}
                )

        # 确定性生成（Fallback）
        outline = generate_outline(req, style_name=final_style_name)
        self.logger.emit(
            session_id, "3.3", "outline_base", outline.model_dump(mode="json")
        )
        ...
        if self.llm.is_enabled():
            try:
                ...
                user_msg = json.dumps(
                    outline.model_dump(mode="json"), ensure_ascii=False
                )
                self.logger.emit(
                    session_id,
                    "3.3",
                    "llm_optimization_prompt",
                    {
                ...
                self._handle_workflow_error(
                    session_id, "3.3", e, {"outline_available": True}
                )

        # 即使LLM未启用优化，也尝试进行类型判断（如果LLM可用）
        if self.llm.is_enabled():
            try:
                outline = await _refine_slide_types(
    ...
    def _post_process_llm_response(
        ...
        if subject_name and knowledge_points:
            ...
            if (
                subject_name in kp_names or len(subject_name) > 10
            ):  # 知识点通常不会这么短
                # 从用户输入中提取学科
                subject_patterns = [
                ...
                for pattern in subject_patterns:
                    ...
                    if match:
                        ...
                        modifications.append(
                            f"修正学科名称：{subject_name} → {new_subject}"
                        )
                        break
        ...
        for kp in knowledge_points:
            if not kp.get("difficulty_level") or kp.get("difficulty_level") not in [
                "easy",
                "medium",
                "hard",
            ]:
                assessed_difficulty = self._assess_knowledge_point_difficulty(
                    kp.get("name", ""), user_text
                )
                if assessed_difficulty != kp.get("difficulty_level"):
                    kp["difficulty_level"] = assessed_difficulty
                    modifications.append(
                        f"评估知识点'{kp['name']}'难度为：{assessed_difficulty}"
                    )
        ...
        if (
            not teaching_scenario.get("scene_type")
            or teaching_scenario.get("scene_type") == "unknown"
        ):
    ...
    def _assess_teaching_scenario(
        self, user_text: str, knowledge_points: List[Dict[str, Any]]
    ) -> str:
        """智能识别教学场景"""
        text_lower = user_text.lower()
    ...
    def _should_reoptimize_with_llm(
        self, answers: Dict[str, Any], original_request: Dict[str, Any]
    ) -> bool:
        """判断是否需要重新调用LLM进行优化"""
        # 检查是否提供了重要的补充信息
        important_keys = [
    ...
    async def _reoptimize_with_llm(
        self,
        session_id: str,
        current_request: TeachingRequest,
...
只输出JSON对象，不要解释。"""
  ...
  "confidence_score": "number"
        ...
        try:
            ...
            self.logger.emit(
                session_id, "3.1", "reoptimize_llm_error", {"error": str(e)}
            )
            # 如果优化失败，返回原请求
            return current_request

    def _apply_optimizations(
        self, request: TeachingRequest, optimizations: Dict[str, Any]
    ) -> TeachingRequest:
        """应用LLM的优化建议"""
                ...
                if field == "knowledge_points.difficulty_level":
                    ...
                    for kp in request.knowledge_points:
                        if kp.name in str(new_value):
                            kp.difficulty_level = new_value.get(
    ...
    async def _final_intent_optimization(
        self, session_id: str, current_request: TeachingRequest
    ) -> Optional[TeachingRequest]:
        """基于用户完整输入进行最终的意图理解优化"""
...
只输出JSON对象，不要解释。"""
  ...
  "confidence_score": "number"
        ...
        context_data = {
            ...
            "all_user_interactions": getattr(
                current_request, "interaction_history", []
            ),
        ...
        try:
            ...
            if parsed and "optimized_request" in parsed:
                ...
                optimized_request = TeachingRequest.model_validate(
                    parsed["optimized_request"]
                )
                # 保持原有的元数据
            ...
            self.logger.emit(
                session_id, "3.1", "final_optimization_llm_error", {"error": str(e)}
            )
    ...
    async def run(
        ...
        auto_fill_defaults_flag: bool,
        stop_at: Optional[str] = None,
        style_name: Optional[str] = None,
        intent_params: Optional[Dict[str, Any]] = None,
    ) -> Tuple[SessionState, str, List[Any]]:
        ...
        if state.teaching_request is None:
            ...
            req = await self._parse_intent(
                ...
                instructions=intent_params.get("instructions")
                if intent_params
                else None,
            )
        ...
        if answers:
            ...
            if self.llm.is_enabled() and self._should_reoptimize_with_llm(
                answers, original_request
            ):
                try:
                    state.teaching_request = await self._reoptimize_with_llm(
                        session_id, state.teaching_request, answers
                    )
                    self.logger.emit(
                        session_id,
                        "3.1",
                        "reoptimized_with_llm",
                        {"answers_provided": answers, "optimizations_applied": True},
                    )
                except Exception as e:
                    self.logger.emit(
                        session_id, "3.1", "reoptimize_error", {"error": str(e)}
                    )
                    # 如果重新优化失败，继续使用原有逻辑

            # Regenerate display summary after updates
            state.teaching_request.display_summary = generate_display_summary(
        ...
        if (
            state.teaching_request.interaction_stage == "confirm_goals"
            and check_slide_count_conflict(state.teaching_request)
            and self.llm.is_enabled()
        ):
            # 如果还没有推荐结果，调用LLM推荐
            if state.teaching_request.slide_requirements.llm_recommended_count is None:
                try:
                    (
                        recommended_count,
                        explanation,
                    ) = await recommend_slide_count_with_llm(
                        state.teaching_request,
                        self.llm,
                        self.logger,
                        session_id,
                    ...
                    if recommended_count:
                        ...
                        state.teaching_request.interaction_metadata[
                            "_llm_recommendation_explanation"
                        ] = explanation
        ...
        if state.teaching_request.interaction_metadata.get(
            "_llm_recommendation_explanation"
        ):
            explanation = state.teaching_request.interaction_metadata[
                "_llm_recommendation_explanation"
            ]
            for q in questions:
        ...
        if auto_fill_defaults_flag:
            ...
            has_additional_info = getattr(
                state.teaching_request, "interaction_metadata", {}
            ).get("has_additional_kps", False) or getattr(
                state.teaching_request, "interaction_metadata", {}
            ).get("has_config_modification", False)

            if has_additional_info and self.llm.is_enabled():
                try:
                    final_optimized = await self._final_intent_optimization(
                        session_id, state.teaching_request
                    )
                    if final_optimized:
                        ...
                        self.logger.emit(
                            ...
                            {"complete_request": True, "has_additional_info": True},
                        )
                except Exception as e:
                    self.logger.emit(
                        session_id, "3.1", "final_optimization_error", {"error": str(e)}
                    )
                    # 如果最终优化失败，继续使用当前结果
            ...
            self.logger.emit(
                ...
                "3.1",
                "autofill_defaults",
                state.teaching_request.model_dump(mode="json"),
            )
            self.store.save(state)
        ...
        if state.deck_content is None:
            ...
            if not ok:
                ...
                self.logger.emit(
                    session_id, "3.4", "validate_ok", {"pages": len(deck.pages)}
                )
    ...
    async def refine_style(
        self, session_id: str, feedback: str
    ) -> Tuple[Optional[StyleConfig], List[StyleSampleSlide], List[str], str]:
        """
        ...
        try:
            ...
            self.logger.emit(
                session_id, "3.2", "refine_engine_error", {"error": str(e)}
            )
            raise e
